...
GitHubでは `manual-jev-decision-evidence` をmainから手動実行できます。利用前にリポジトリのActions Secret `TYPESAFE_API_KEY` を登録してください。このワークフローはPRやpushでは起動せず、main以外では認証ジョブを実行しません。キーは依存関係のインストール工程へ渡しません。shadowが成功した場合だけliveへ進み、全体の要求数は最大48回です。このワークフローの実API成功を本変更の合成テストで代替することはできません。
