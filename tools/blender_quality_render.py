...
def main():
    ...
    instances=[]
    if not args.source_only and plan['trees']:
        ...
        for item in plan['trees']:
            instance=bpy.data.objects.new('OSM_tree_'+item['osm_node_id'],None)
            instance.instance_type='COLLECTION'; instance.instance_collection=collection
            instance.location=item['xyz']; instance.rotation_euler.z=(int(item['osm_node_id'])%360)*math.pi/180
