...
world = json.loads((ROOT/'world/world.json').read_text(encoding='utf8'))
...
runs = {p.stem: json.loads(p.read_text(encoding='utf8')) for p in sorted((ROOT/'reports').glob('*.json'))
...
template = (ROOT/'web/template.html').read_text(encoding='utf8')
output = template.replace('/*BUNDLED_DATA*/{}', data)
(ROOT/'web/index.html').write_text(output, encoding='utf8')
